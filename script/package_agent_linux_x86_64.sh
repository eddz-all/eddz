...
PACKAGE_DIR="$DIST_ROOT/projectpilot-agent-linux-x86_64"
ARCHIVE="$DIST_ROOT/projectpilot-agent-linux-x86_64.tar.gz"
...
echo "==> Building Linux x86_64 static TUI binary"
...
# ProjectPilot Agent for Linux x86_64

This package is intended to be copied to an Ubuntu x86_64 server. It does not
require Rust, Python, a virtualenv, or source code on the customer machine.

## Install

```bash
tar -xzf projectpilot-agent-linux-x86_64.tar.gz
cd projectpilot-agent-linux-x86_64
./install.sh
```
...
echo "==> Linux x86_64 package ready:"
echo "    $ARCHIVE"
