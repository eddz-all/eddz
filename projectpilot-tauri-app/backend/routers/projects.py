...
from models import EnvironmentSnapshot, GitStatus, OperationLog, Project, ProjectServerMapping
...
def delete_project(project_id: int, db: Session = Depends(get_db)):
    ...
    db.query(OperationLog).filter(OperationLog.project_id == project_id).delete(
        synchronize_session=False
    )
    db.query(EnvironmentSnapshot).filter(
        EnvironmentSnapshot.project_id == project_id
    ).delete(synchronize_session=False)
    db.query(GitStatus).filter(GitStatus.project_id == project_id).delete(
        synchronize_session=False
    )
