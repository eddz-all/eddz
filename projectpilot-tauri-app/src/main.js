...
function simulatedAnalysisDelay(minMs = 1800, maxMs = 3200) {
  const duration = minMs + Math.round(Math.random() * (maxMs - minMs));
...
function defaultLocalDemoData() {
  ...
  return {
    ...
    projects: [
      {
        ...
        path: demoRoot,
        description: "Course showcase workspace with rich branch, tag, conflict, detached HEAD, and dirty worktree states.",
    ...
    servers: demoRepositories.map((repo) => ({
        ...
        description: `Course showcase workspace: ${repo.slug}`,
        created_at: createdAt
      })),
    ...
    operationLogs: [
      ...
      },
      {
        ...
        detail: "The presentation dataset opens the test workspace by default.",
...
function applyLocalDemoMigrations(data) {
  ...
  data.servers.forEach((server) => {
    server.connection_mode = server.connection_mode || "local";
    if (/local demo workspace/i.test(server.description || "")) {
  ...
  data.operationLogs.forEach((log) => {
    ...
    if (/Generated local smart Git analysis/i.test(log.summary || "")) {
      log.summary = "Generated smart Git analysis";
    }
    if (/local executor-safe/i.test(log.summary || "")) {
      log.summary = "Queued executor-safe action";
    }
  });
...
function localCreateDetection(data, projectId, serverId) {
  ...
  localAddOperationLog(data, {
    ...
    detail: "Generated by the presentation workflow."
  });
...
function localAnalysis(data, projectId) {
  ...
  return {
    ...
    issues: [
      serverCount ? `${serverCount} 个工作区已进入健康状态矩阵` : "当前项目还没有绑定工作区",
      dockerIssues ? `${dockerIssues} 个环境的 Docker 未运行` : "环境快照没有发现阻塞项",
      "当前使用课程演示数据集"
    ],
    recommendations: [
      "优先验证项目、服务器、绑定、检测、任务流闭环",
      "课程汇报时先展示 Git Workspace，再展示 AI Ops 和任务流"
    ]
  };
}
...
function localGitWorktree(data, projectId) {
  ...
  const repositories = status.servers.map((server, index) => {
    ...
    const graphCommits = demoGraph?.commits || [
      {
        ...
        parents: [parentHash],
        subject: git.last_commit || "Workspace HEAD",
        author: "ProjectPilot",
    ...
    return {
      ...
      graph_text: demoGraph?.graph_text || [`* ${headHash.slice(0, 7)} (${git.branch || "detached"}) ${git.last_commit || "Workspace HEAD"}`, `* ${parentHash.slice(0, 7)} Baseline workspace state`],
      worktree: {
  ...
  return {
    ...
    project_id: projectId,
    project_name: project?.name || "ProjectPilot Demo",
    repositories
  };
}
...
function localPlan(data, projectId, body = {}) {
  ...
  return {
    ...
    goal: body.goal || "Validate ProjectPilot presentation workflow",
    status: "ready",
    summary: "Analysis plan is ready for executor handoff.",
    steps: [
      {
        order: 1,
...
function localMarkdownReport(data, projectId) {
  ...
  return `# ${project?.name || "ProjectPilot"} AI Workspace Report

| Area | Current State |
| --- | --- |
| Dataset | Course showcase |
| Bound servers | ${status.servers.length} |
| Executor tasks | ${tasks.length} |

## Main Flow

1. Project registry is ready for the course demo.
2. Server records can be created and selected.
3. Project-server bindings are persisted for repeatable presentation.
...
function localRequest(path, options = {}) {
  ...
  if (method === "POST" && parts.length === 3 && parts[0] === "servers" && parts[2] === "check-connection") {
    ...
    localAddOperationLog(data, {
      ...
      detail: "Connection check completed."
    });
    return commit({
      server_id: server.id,
      server_name: server.name,
  ...
  if (method === "DELETE" && parts.length === 4 && parts[0] === "projects" && parts[2] === "servers") {
    ...
    return commit({ message: "Project-server binding deleted" });
  }
  ...
  if (method === "GET" && parts.length === 2 && parts[0] === "ai" && parts[1] === "settings") {
    return {
      provider: "ProjectPilot AI",
      model: "workspace-showcase",
  ...
  if (method === "POST" && parts.length === 4 && parts[0] === "projects" && parts[2] === "ai" && parts[3] === "analyze-git") {
    ...
    localAddTask(data, {
      ...
      message: "Smart Git analysis completed",
      result: analysis
    });
    localAddOperationLog(data, {
      project_id: Number(parts[1]),
      ...
      summary: "Generated smart Git analysis",
  ...
  if (method === "POST" && parts.length === 4 && parts[0] === "projects" && parts[2] === "ai" && parts[3] === "plan-action") {
    ...
    const response = {
      ...
      message: body.auto_execute ? "Executor task queued" : "Action plan ready",
    ...
    if (body.auto_execute) {
      ...
      localAddOperationLog(data, {
        ...
        risk_level: "medium",
        summary: "Queued executor-safe action"
      });
    }
  ...
  if (
    ...
    const task = localAddTask(data, {
      ...
      task_type: "execute_config_plan",
      status: "completed",
      message: "Config plan execution completed",
      result: {
        success: true,
    ...
    return commit({
      ...
      results: [{ status: "completed", message: "Simulated execution completed" }]
    });
  }
...
function renderMarkdown(markdown) {
  if (!markdown) {
    return `<p class="muted-copy">暂无报告内容</p>`;
  }
...
async function request(path, options = {}, fallback) {
  ...
  if (typeof fallback === "function") {
    return fallback(new Error("Demo endpoint unavailable"));
  }
  return fallback;
}
  ...
  return `
    <tr>
      <td>
        ...
        <span>${escapeHtml(server.host)}:${escapeHtml(server.port)} · ${escapeHtml(displayConnectionMode(server.connection_mode))}</span>
      </td>
                  ...
                  (server) => {
                    ...
                    return `
                      <tr>
                        ...
                        <td><span class="badge muted">${escapeHtml(displayConnectionMode(server.connection_mode))}</span></td>
        ...
        <form class="stack-form" data-server-form data-draft-form="server">
          ...
          <label>
            ...
            <select name="connection_mode">
              ...
              <option value="managed" ${state.drafts.server.connection_mode === "managed" ? "selected" : ""}>managed</option>
            </select>
          </label>
          <label>
            <span>说明</span>
    ...
    <div class="report-layout">
      <section class="panel">
        ...
        <div class="key-value-list">
          ...
          <span>Mode</span><strong>${escapeHtml(displayConnectionMode(server.connection_mode))}</strong>
...
function renderBindingRow(binding) {
  ...
  const mode = displayConnectionMode(binding.connection_mode || server.connection_mode);
    ...
    <div class="two-column">
      <section class="panel">
        ...
        <div class="key-value-list">
          ...
          <span>Workspace</span><strong>Git graph showcase</strong>
        </div>
      </section>
      <section class="panel">
        <div class="panel-header">
      ...
      <section class="panel">
        ...
        <p class="muted-copy">当前是课程演示会话。这里展示汇报账号、角色和桌面端运行边界。</p>
...
function bindShell() {
  ...
  document.querySelector("[data-refresh]")?.addEventListener("click", async () => {
    await runAction("refresh-data", "正在刷新演示数据", async () => {
      await loadData({ silent: true });
    });
  });

  document.querySelector("[data-use-local-demo]")?.addEventListener("click", async () => {
    await runAction("use-local-demo", "正在加载演示数据", async () => {
      await activateLocalDemo("演示数据已启用");
    });
  });

  document.querySelector("[data-retry-backend]")?.addEventListener("click", async () => {
    await runAction("retry-backend", "正在刷新演示数据", async () => {
      state.localDemoActive = true;
      state.backendMode = "local";
      state.backendIssue = "";
      await loadData({ silent: true });
...
async function activateLocalDemo(message = "演示数据已启用") {
  state.localDemoActive = true;
  state.backendMode = "local";
  state.backendIssue = "";
  setToast(message);
...
async function handleSettings(event) {
  ...
  await runAction("save-settings", "正在保存设置", async () => {
...
async function handleReport() {
  const projectId = projectIdForActions();
  if (!projectId) {
    setToast("当前没有可用项目，无法生成报告");
    return;
  }
  await runAction("generate-report", "AI 正在整理项目报告", async () => {
    await simulatedAnalysisDelay(1700, 2800);
...
async function handleRefreshAi() {
  const projectId = projectIdForActions();
  if (!projectId) {
    setToast("当前没有可用项目，无法刷新 AI");
    return;
  }
  await runAction("refresh-ai", "AI 正在分析环境健康状态", async () => {
    await simulatedAnalysisDelay(1600, 2800);
...
async function handleGenerateConfigPlan() {
  ...
  await runAction("config-plan", "AI 正在推演配置计划", async () => {
    await simulatedAnalysisDelay(1800, 3200);
    const result = await request(
...
async function handleAnalyzeGit() {
  const projectId = projectIdForActions();
  if (!projectId) {
    setToast("当前没有可用项目，无法分析 Git");
    return;
  }

  await runAction("analyze-git", "AI 正在分析 Git 工作区", async () => {
    await simulatedAnalysisDelay(1900, 3400);
    const result = await request(
...
async function handleServerDetail(serverId) {
  if (!serverId) {
    setToast("当前没有可用 server id，无法读取服务器详情");
    return;
  }
...
async function handleTaskDetail(taskId) {
  if (!taskId) {
    setToast("当前没有可用 task id，无法读取详情");
    return;
  }
...
async function handleExecutePlan() {
  ...
  const projectId = projectIdForActions();
  if (!projectId) {
    setToast("当前没有可用项目，无法执行");
    return;
  }
  const steps = planSteps(state.plan);
  if (!steps.length) {
    setToast("当前没有可执行的配置步骤");
    return;
  }
