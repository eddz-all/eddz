...
- 必须有 `approved: true`；
- 后端可传 `expected_command`，Executor 必须逐字匹配；
- 本地任务必须在 `allowed-root` 内；
- 远程任务必须使用 SSH Host 和绝对项目路径；
- 不允许 raw shell 命令。
...
| 4. 安全 Git 执行 | Git 操作可审批执行 | `apply_git_operation`、`apply_remote_git_operation`、`approved` 校验、`expected_command` 校验、白名单命令、执行前后快照 | 用户批准后执行成功，失败可解释 |
...
| 后端 | `/executor/poll`、`/executor/tasks/{task_id}/result`、创建检测任务和已批准 Git 执行任务 |
| Executor | `check_connection`、`detect_remote_git_status`、`detect_remote_environment`、`apply_git_operation`、`apply_remote_git_operation` |
| 前端 | 服务器列表、检测按钮、Git 状态、环境状态、计划审批和执行结果 |
